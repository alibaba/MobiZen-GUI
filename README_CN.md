...
- **40 亿参数的智能体[模型](https://huggingface.co/alibabagroup/MobiZen-GUI-4B)**：可完全在本地台式机或笔记本电脑上运行。  
- **仅依赖单图像加历史动作，执行速度快**：仅依赖单张当前图像加历史动作，无需额外信息，执行速度快。
- **即插即用的推理套件**：自动处理 ADB 连接并安装所有依赖库。
- **[CMGUI](https://huggingface.co/datasets/alibabagroup/CMGUI)**：面向中文移动 GUI 智能体的大规模高质量数据集——覆盖电商、社交、本地服务等50余款真实应用，含18K轨迹、98K步操作，全部经人工校验并标注精准边框。
- **[CMGUI-Bench](https://huggingface.co/datasets/alibabagroup/CMGUI)**：汇集44款热门中国应用的386条人工验证导航任务、2547步操作，提供多选动作标注，用于评估多种有效GUI交互场景下的智能体表现。
